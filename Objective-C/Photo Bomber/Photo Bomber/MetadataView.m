...
- (void)setPhoto:(NSDictionary *)photo {
    ...
    NSLog(@"Data: %@", _photo);
    [PhotoController avatarForPhoto:_photo completion:^(UIImage *image) {
...
- (void)openUser:(id)sender {
    NSString *urlString = [[NSString alloc] initWithFormat:@"https://instagram.com/%@", self.usernameButton.titleLabel.text];
...
- (void)openPhoto:(id)sender {
    NSString *urlString = [[NSString alloc] initWithFormat:@"%@", _photo[@"link"]];
    NSURL *url = [NSURL URLWithString:urlString];
...
- (UIImageView *)avatarImageView {
	if (!_avatarImageView) {
		...
		_avatarImageView.backgroundColor = [UIColor colorWithWhite:0.2 alpha:1.0f];

...
//        let urlString = "http://www.xcode.abbasiindustries.com/meetamuslim/token.php?device=\(deviceId)&identity=\(identity)"
        let urlString = "http://www.xcode.abbasiindustries.com/meetamuslim/token.php?device=\(deviceId)"
//        let urlString = "http://www.xcode.abbasiindustries.com/meetamuslim/token.php"
...
//        // POST Parameters
//        var bodyData = "device=\(deviceId)&identity=\(identity)"
//        request.HTTPBody = bodyData.dataUsingEncoding(NSUTF8StringEncoding);
                ...
                self.client?.channelsListWithCompletion { result, channels in
                    if (result == .Success) {
                        if let channel = channels.channelWithUniqueName(defaultChannel) {
                            ...
                            channels.createChannelWithFriendlyName(defaultChannel, type: .Public) {
    ...
    func loadMessages() {
        ...
        if let messages = self.generalChannel?.messages.allObjects() {
